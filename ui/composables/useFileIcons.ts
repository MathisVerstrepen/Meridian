// This code is from the vscode-material-icon-theme repository
...
type LightSettingsWithCloneOptions =
    | {
...
type FileIcon = RequireAtLeastOneFileIcon<BasicFileIcon>;
...
type FileIconWithPatterns = (FileIcon & { patterns?: Patterns })[];
...
const parseByPattern = (rawFileIcons: FileIconWithPatterns): FileIcon[] => {
...
const fileIcons: FileIcons = {
    defaultIcon: { name: 'MdiFileOutline' },
    icons: parseByPattern([
        ...
        { name: 'diff', fileExtensions: ['diff', 'patch'] },
        {
            ...
            fileNames: ['action.yml', 'action.yaml', 'workflow.yml', 'workflow.yaml'],
        },
        { name: 'matlab', fileExtensions: ['m', 'mat'] },
        { name: 'objective-c', fileExtensions: ['m'] },
        { name: 'objective-cpp', fileExtensions: ['mm'] },
        { name: 'salesforce', fileExtensions: ['cls', 'trigger', 'apex'] },
        { name: 'silverstripe', fileExtensions: ['ss'] },
        {
            name: 'systemd',
            fileExtensions: [
                'service',
...
export const useFileIcons = () => {
    const getIconForFile = (filePath: string): string | null => {
        ...
        for (const icon of fileIcons.icons) {
            if (icon.fileNames?.includes(filename.toLowerCase())) {
                return icon.name;
            }
        }

        for (let i = 0; i < extensions.length; i++) {
            const extension = extensions.slice(i).join('.');
            for (const icon of fileIcons.icons) {
                if (icon.fileExtensions?.includes(extension.toLowerCase())) {
                    return icon.name;
                }
            }
        }

    ...
    <div v-if="jobs.length" class="mb-6">
        ...
        <div class="active-lane custom_scroll flex gap-3 overflow-x-auto pb-2">
            <div
                v-for="job in jobs"
                :key="job.id"
                class="bg-obsidian/60 active-card relative w-52 shrink-0 overflow-hidden rounded-2xl border"
                :class="job.status === 'failed' ? 'border-red-400/45 bg-red-950/20' : 'border-stone-gray/15'"
            >
                <div
                    class="developing-card relative flex aspect-square items-center justify-center overflow-hidden"
                    :class="{
                ...
                >
                    ...
                    <div class="relative z-10 flex flex-col items-center gap-2 px-3 text-center">
                        <div
                            ...
                            class="developing-spinner border-soft-silk/15 border-t-ember-glow h-9 w-9
                                animate-spin rounded-full border-2"
                        />
                        <div
                            v-else
                            class="flex h-10 w-10 items-center justify-center rounded-xl bg-red-500/15
                                text-red-200 ring-1 ring-red-300/25 ring-inset"
                        >
                            <UiIcon name="PhImageBroken" class="h-6 w-6" />
                        </div>
                        <span
                            class="font-mono text-[10px] tracking-widest uppercase"
                            :class="job.status === 'failed' ? 'text-red-200/85' : 'text-soft-silk/80'"
                        >
                            {{ job.status === 'failed' ? 'Failed' : job.status === 'retrying' ? 'Retrying' : job.status }}
                        </span>
                ...
                <div class="p-2.5">
                    <p class="text-soft-silk truncate text-[11px] font-semibold">{{ job.prompt }}</p>
                    ...
                    <div v-if="job.status === 'failed'" class="mt-2 grid grid-cols-2 gap-1.5">
                        <button
                            ...
                            class="rounded-lg border border-red-300/25 bg-red-500/10 px-2 py-1
                                text-[10px] font-semibold text-red-100 transition hover:bg-red-500/20"
                        ...
                        <button
                            type="button"
                            class="border-stone-gray/15 text-stone-gray rounded-lg border bg-black/20 px-2
                                py-1 text-[10px] font-semibold transition hover:border-red-300/30
                                hover:text-red-100"

...
const typedArguments = computed<ToolQuestionArguments | null>(() =>
    normalizeToolQuestionArguments(props.detail.arguments),
);
const answeredItems = computed<AnsweredItem[]>(() => normalizeAnsweredItems(props.detail.result));
...
const canMoveForward = computed(() => {
    if (!currentQuestion.value || !isPending.value || props.isSubmitting) {
        return false;
    }
...
const canSubmit = computed(() => {
    if (!typedArguments.value || !isPending.value || props.isSubmitting) {
        return false;
    }
...
const syncQuestionState = (
    ...
    for (const question of questions) {
        nextSingleValues[question.id] = preserveExisting ? singleValues.value[question.id] || '' : '';
        nextMultiValues[question.id] = preserveExisting
            ? [...(multiValues.value[question.id] || [])]
            : [];
        nextBooleanValues[question.id] = preserveExisting
            ? question.id in booleanValues.value
                ? booleanValues.value[question.id]
                : null
            : null;
        nextTextValues[question.id] = preserveExisting ? textValues.value[question.id] || '' : '';
        nextOtherTextValues[question.id] = preserveExisting
            ? otherTextValues.value[question.id] || ''
            : '';
        nextNoteValues[question.id] = preserveExisting ? noteValues.value[question.id] || '' : '';
...
watch(
    typedArguments,
    (value) => {
        syncQuestionState(value?.questions ?? [], true);
    },
    { immediate: true },
);
...
<template>
    <div
        ...
        class="tq-card group border-stone-gray/10 bg-anthracite/30 overflow-hidden rounded-xl
            border backdrop-blur-sm"
    >
        <template v-if="typedArguments && currentQuestion">
            <div
                ...
                >
                    <textarea
                        ...
                        class="border-stone-gray/12 bg-obsidian/25 text-soft-silk
                            placeholder:text-stone-gray/30 focus:border-ember-glow/35
                            focus:ring-ember-glow/10 min-h-19.5 w-full rounded-lg border px-3

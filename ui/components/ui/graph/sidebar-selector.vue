...
<template>
    <div
        class="bg-anthracite/75 border-stone-gray/10 absolute top-2 right-2 z-10 flex h-[calc(100%-1rem)] w-0
            flex-col items-center justify-start rounded-2xl border-2 px-4 py-10 shadow-lg backdrop-blur-md
            transition-[width] duration-300 ease-in-out"
        :class="{ 'w-[30rem]': isOpen }"
    >
        <div class="w-full overflow-hidden">
            <div
                class="flex w-[28rem] flex-col items-center transition-opacity duration-300 ease-in-out"
                :class="{
                    'opacity-0': !isOpen,
                    'opacity-100': isOpen,
                }"
            >
                ...
                <div
                    class="flex w-full flex-col items-center justify-start overflow-y-auto px-4 pb-10"
                >
                    <HeadlessDisclosure

...
def upgrade() -> None:
    ...
    op.create_index('idx_provider_tokens_user_provider', 'provider_tokens', ['user_id', 'provider'], unique=True)
    op.create_index(op.f('ix_provider_tokens_user_id'), 'provider_tokens', ['user_id'], unique=False)
    op.create_index(op.f('ix_provider_tokens_provider'), 'provider_tokens', ['provider'], unique=False)
    ...
    op.create_index('idx_repositories_user_repo_provider', 'repositories', ['user_id', 'repo_name', 'provider'], unique=True)
    op.create_index(op.f('ix_repositories_user_id'), 'repositories', ['user_id'], unique=False)
    op.create_index(op.f('ix_repositories_local_path_uuid'), 'repositories', ['local_path_uuid'], unique=True)
    op.create_table('settings',
    sa.Column('id', postgresql.UUID(as_uuid=True), server_default=sa.text('uuid_generate_v4()'), nullable=False),
    sa.Column('user_id', postgresql.UUID(as_uuid=True), nullable=False),
    sa.Column('settings_data', postgresql.JSONB(astext_type=sa.Text()), server_default='{}', nullable=False),
    sa.Column('created_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=False),
    sa.Column('updated_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=False),
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], ),
    sa.PrimaryKeyConstraint('id')
    ...
    op.create_index(op.f('ix_used_refresh_tokens_token'), 'used_refresh_tokens', ['token'], unique=True)
    ...
    sa.ForeignKeyConstraint(['graph_id', 'source_node_id'], ['nodes.graph_id', 'nodes.id'], ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['graph_id', 'target_node_id'], ['nodes.graph_id', 'nodes.id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('graph_id', 'id')
    )

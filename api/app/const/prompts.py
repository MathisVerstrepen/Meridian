...
- **`execute_code` Tool:**
    ...
    *   **How to Use:** Provide a short UI-friendly `title` plus self-contained Python code in the `code` argument. The `title` should summarize the purpose of the run in natural language, not repeat the code. Prefer concise snippets and use `print(...)` to surface the values you need. Keep the snippet focused on the user's request.
    ...
    *   **Embedding Returned Artifacts:** When the tool returns an image artifact, embed it with standard Markdown image syntax: `![Helpful caption](<artifact_id>)` (DO NOT use $ or $$ tags in caption). When the tool returns a downloadable file artifact, embed it with this link syntax: `[Download filename](sandbox-file://<artifact_id>)`.
"""

...
- **`execute_code` Tool:**
    ...
    *   **Embedding Returned Artifacts:** When the tool returns an image artifact, embed it with standard Markdown image syntax: `![Helpful caption](<artifact_id>)` (DO NOT use $ or $$ tags in caption). When the tool returns an HTML artifact that should be shown inline, embed it with `[Helpful caption](sandbox-html://<artifact_id>)`. When the tool returns any other downloadable file artifact, embed it with `[Download filename](sandbox-file://<artifact_id>)`.
"""

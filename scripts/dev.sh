...
generate_frontend_env() {
    ...
    tmp_file="$(mktemp)"
    {
        printf '# Generated by ./scripts/dev.sh from docker/config.local.toml.\n'
        printf '# Edit docker/config.local.toml and rerun ./scripts/dev.sh instead of editing this file.\n'
    } > "$tmp_file"
    ...
    if (( ${#missing_keys[@]} > 0 )); then
        rm -f "$tmp_file"
        err "Missing required frontend env keys in docker/config.local.toml: ${missing_keys[*]}"
        return 1
    fi

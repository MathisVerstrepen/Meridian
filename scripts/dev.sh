...
generate_frontend_env() {
    local docker_env_file="$DOCKER_DIR/env/.env.local"
    local frontend_env_file="$UI_DIR/.env"
    local tmp_file
    ...
    if [[ ! -f "$docker_env_file" ]]; then
        err "Docker env file not found: $docker_env_file"
        return 1
    fi

    tmp_file="$(mktemp)"
    {
        printf '# Generated by ./scripts/dev.sh from docker/env/.env.local.\n'
        printf '# Edit docker/config/overrides/local.yaml or docker/config/secrets/local.env and rerun ./scripts/dev.sh.\n'
    } > "$tmp_file"
    ...
    for required_key in "${required_keys[@]}"; do
        if [[ -z "${generated_keys[$required_key]:-}" ]]; then
    ...
    if (( ${#missing_keys[@]} > 0 )); then
        rm -f "$tmp_file"
        err "Missing required frontend env keys in docker/env/.env.local: ${missing_keys[*]}. Run docker/run.sh dev --config-only."
        return 1
    fi

    mv "$tmp_file" "$frontend_env_file"
    ok "Frontend env generated ($frontend_env_file)"
}
...
log "Generating frontend env from docker config..."
generate_frontend_env
